...
function agentIdFrom(req: express.Request): string {
  return (
    truncate(req.get('x-agent-id'), 120) ||
    truncate(req.get('x-forwarded-user'), 120) ||
    truncate(req.get('x-user-email'), 120) ||
    truncate(req.ip, 120) ||
    'anonymous'
  );
}

function identityFrom(req: express.Request) {
  const externalId = agentIdFrom(req);
  return {
    externalId,
    email: truncate(req.get('x-user-email'), 240) || (externalId.includes('@') ? externalId : null),
    name: truncate(req.get('x-user-name'), 240) || null,
  };
}
...
app.post('/maintenance/prune', (_req, res) => {
  ...
  <main>
    ...
    <section class="split">
      ...
      <div class="card">
        ...
        <form id="user-form" class="form-grid">
          <label class="form-row">External ID<input name="externalId" placeholder="agent@example.com" required /></label>
    ...
    <section class="card" style="margin-top:12px">
      ...
      <form id="ask-form" class="form-grid">
        <label class="form-row">Agent ID<input name="agentId" value="dashboard-demo" /></label>
  ...
  <script>
    ...
    document.getElementById('ask-form').addEventListener('submit', async (event) => {
      ...
      const res = await fetch('/ask', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-agent-id': data.agentId || 'dashboard-demo' },
        body: JSON.stringify({
          query: data.query,
...
app.post('/ask', async (req, res) => {
  const settings = runtimeSettings();
  const user = resolveUser(identityFrom(req));
